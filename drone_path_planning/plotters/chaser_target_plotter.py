...
_CAUGHT_ONE_OUTCOME: str = 'intercepted some'
...
class ChaserTargetPlotter(Plotter):
    ...
    def _find_min_chaser_target_distances(self, trajectories: _TrajectoryList) -> np.ndarray:
        ...
        if _HAS_OBSTACLES:
            for trajectory in trajectories:
                ...
                if last_environment_state[HAS_COLLIDED] or last_environment_state[HAS_CRASHED] or not last_environment_state[HAS_INTERCEPTED]:
                    continue
                min_chaser_target_distances.extend(map(self._find_min_chaser_target_distance, trajectory))
        else:
            for trajectory in trajectories:
                last_environment_state = trajectory[-1]
                if last_environment_state[HAS_COLLIDED] or not last_environment_state[HAS_INTERCEPTED]:
                    continue
                min_chaser_target_distances.extend(map(self._find_min_chaser_target_distance, trajectory))
    ...
    def _plot_animation(
        ...
        func_animation.save(save_filepath, writer=writer)
    ...
    def _create_animation_frame_updater(
        ...
        def update(frame_index: int) -> Iterable[artist.Artist]:
            ...
            chaser_quiver = ax.quiver(
                ...
                length=arrow_length,
                normalize=True,
                colors=[(0.0, 1.0, 0.0, 0.9),(0.0, 0.0, 1.0, 0.9),(0.0, 0.0, 0.0, 0.9),(1.0, 1.0, 0.0, 0.9)],
            )
