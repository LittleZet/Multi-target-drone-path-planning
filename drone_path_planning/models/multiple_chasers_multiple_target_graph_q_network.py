...
from drone_path_planning.utilities.constants import TARGET_TARGET
...
from drone_path_planning.utilities.constants import OBSTACLE
...
from drone_path_planning.utilities.constants import AGENT_OBSTACLE
from drone_path_planning.utilities.constants import OBSTACLE_AGENT
from drone_path_planning.utilities.constants import CHASER_OBSTACLE
from drone_path_planning.utilities.constants import OBSTACLE_CHASER
...
@tf.keras.utils.register_keras_serializable('dpp.models', 'cn_t2_gqn')
class MultipleChasersMultipleTargetGraphQNetwork(tf.keras.Model):
    def __init__(
        self,
    ...
    def _build_graph(self, inputs: Dict[str, tf.Tensor]) -> Graph:
        ...
        #target_target_edge_set = self._build_all_pairs_with_distinct_elements_relative_displacement_edge_set(target_relative_displacements, targets, TARGET)
        
        #Add obstacle nodes and edges if has obstacles
        if has_obstacles:
            obstacle_relative_displacements = inputs[OBSTACLE_RELATIVE_DISPLACEMENTS]
            num_obstacles = tf.shape(obstacle_relative_displacements)[0]
            obstacles = tf.range(num_obstacles)
            agents_for_obstacles = tf.zeros_like(obstacles)
            obstacle_node_set = NodeSet(tf.ones([num_obstacles,1]))
            agent_obstacle_edge_set = self._build_edge_set(obstacle_relative_displacements, agents_for_obstacles, obstacles, AGENT, OBSTACLE)
            obstacle_agent_edge_set = self._build_edge_set(-obstacle_relative_displacements, obstacles, agents_for_obstacles, OBSTACLE, AGENT)
            chaser_obstacle_edge_set = self._build_all_pairs_relative_displacement_edge_set(chaser_relative_displacements, obstacle_relative_displacements, chasers, obstacles, CHASER, OBSTACLE)
            obstacle_chaser_edge_set = self._build_all_pairs_relative_displacement_edge_set(obstacle_relative_displacements, chaser_relative_displacements, obstacles, chasers, OBSTACLE, CHASER)
            
            node_sets = {
                AGENT: agent_node_set,
                CHASER: chaser_node_set,
                TARGET: target_node_set,
                OBSTACLE: obstacle_node_set,
            }
            ...
            edge_sets = {
                ...
                AGENT_OBSTACLE: agent_obstacle_edge_set,
                OBSTACLE_AGENT: obstacle_agent_edge_set,
                CHASER_OBSTACLE: chaser_obstacle_edge_set,
                OBSTACLE_CHASER: obstacle_chaser_edge_set,
                #TARGET_TARGET: target_target_edge_set,
            }
        ...
        with open('graph log.txt','w') as g:
            g.write('\n')
            g.write('\n')
            g.write(str(graph))
            g.write('\n')
    ...
    def _build_all_pairs_relative_displacement_edge_set(
        ...
        edge_features = tf.ensure_shape(edge_features, [None, 4])
        edge_set = EdgeSet(
            edge_features,
            all_pairs_senders,
            all_pairs_receivers,
        ...
        # print()
        # tf.print(sender_receiver_pairs)
        # tf.print(sender_set)
        # tf.print(receiver_set)
        # print()
