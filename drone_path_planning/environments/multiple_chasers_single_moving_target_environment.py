...
class MultipleChasersSingleMovingTargetEnvironment(MultiAgentEnvironment):
    ...
    def reset(self): #Resetting after each episode
        ...
        if _HAS_OBSTACLES:
            for _ in range(1, self._num_obstacles):
                ...
                while self._has_any_within_threshold_distance(_CHASER_SELF_COLLISION_DISTANCE, self._chaser_displacements, obstacle_displacement) or self._has_any_within_threshold_distance(_CHASER_SELF_COLLISION_DISTANCE, self._chaser_displacements, obstacle_displacement):
                    obstacle_displacement = self._create_initial_obstacle_displacements(1)
    ...
    def update(self): #Update environment after each step
        ...
        if _HAS_OBSTACLES:
            ...
            self._has_collided = self._chaser_hit_obstacle or self._chaser_collided
        else:
