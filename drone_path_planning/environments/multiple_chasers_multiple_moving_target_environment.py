...
from drone_path_planning.utilities.constants import OBSTACLE_RELATIVE_VELOCITIES
...
from drone_path_planning.assignment import task_assignment as assign
...
_INITIAL_BOUNDS_WIDTH: float = 4.0 #Initially 4.0
_INITIAL_BOUNDS_HEIGHT: float = 4.0 #Initially 4.0
_DELTA_T: float = 0.1
...
_CRASHING_REWARD = -20.0

_NUM_CHASERS: int = 3
_CHASER_MASS: List[float] = [i*0.5 for i in range(_NUM_CHASERS,0,-1)] #Initially 1,1,1
_CHASER_TERMINAL_SPEED: List[float] = [i*0.5 for i in range(1,_NUM_CHASERS+1)] #Initially 1,1,1
...
_CHASER_MOMENT_OF_INERTIA: List[float] = [i*0.5 for i in range(_NUM_CHASERS,0,-1)]
_CHASER_TERMINAL_ANGULAR_SPEED: List[float] = [math.pi/(i*2) for i in range(_NUM_CHASERS,0,-1)]
...
_NUM_TARGETS = 2
_TARGET_MASS: float = [i*0.5 for i in range(_NUM_TARGETS,0,-1)]
_TARGET_TERMINAL_SPEED: float = [i*0.249 for i in range(1,_NUM_TARGETS+1)]
_TARGET_ACCELERATION_NORM: float = [_DRAG_COEFFICIENT * _TARGET_TERMINAL_SPEED[i] ** 2 / _TARGET_MASS[i] for i in range(0,len(_TARGET_MASS))]
_TARGET_MOMENT_OF_INERTIA: float = [i*0.5 for i in range(_NUM_TARGETS,0,-1)]
_TARGET_TERMINAL_ANGULAR_SPEED: float = [math.pi/(i*4) for i in range(_NUM_TARGETS,0,-1)]
_TARGET_ANGULAR_ACCELERATION_NORM: float = [_ANGULAR_DRAG_COEFFICIENT * _TARGET_TERMINAL_ANGULAR_SPEED[i] ** 2 / _TARGET_MOMENT_OF_INERTIA[i] for i in range(0,len(_TARGET_MASS))]
...
_HAS_OBSTACLES = True
_ASSIGNMENT_METHOD = 2


class MultipleChasersMultipleMovingTargetEnvironment(MultiAgentEnvironment):
    def __init__(
        self,
        chaser_ids: Set[str],
    ):
        ...
        self._num_targets = _NUM_TARGETS
        self._rewarded = False
        self._targets_intercepted = []
        
        self._target_masses = tf.reshape(tf.convert_to_tensor(_TARGET_MASS, dtype=tf.float32),[-1,1])
        #self._target_masses = tf.fill([self._num_targets, 1], _TARGET_MASS)
        self._target_acceleration_norms = tf.reshape(tf.convert_to_tensor(_TARGET_ACCELERATION_NORM, dtype=tf.float32),[-1,1])
        #self._target_acceleration_norms = tf.fill([self._num_targets, 1], _TARGET_ACCELERATION_NORM)
        self._target_moments_of_inertia = tf.reshape(tf.convert_to_tensor(_TARGET_MOMENT_OF_INERTIA, dtype=tf.float32),[-1,1])
        self._target_angular_acceleration_norms = tf.reshape(tf.convert_to_tensor(_TARGET_ANGULAR_ACCELERATION_NORM, dtype=tf.float32),[-1,1])
        ...
        self._has_updated = tf.constant(False)
    ...
    def reset(self): #Resetting after each episode
        ...
        ################# Generate obstacles###########################
        if _HAS_OBSTACLES:
            self._obstacle_displacements = self._create_initial_obstacle_displacements(1)
            while self._has_any_within_threshold_distance(_CHASER_SELF_COLLISION_DISTANCE, self._chaser_displacements, self._obstacle_displacements):
                    self._obstacle_displacements = self._create_initial_obstacle_displacements(1)
                    
            for _ in range(1, self._num_obstacles):
                obstacle_displacement = self._create_initial_obstacle_displacements(1)
                while self._has_any_within_threshold_distance(_CHASER_SELF_COLLISION_DISTANCE, self._chaser_displacements, obstacle_displacement):
                    obstacle_displacement = self._create_initial_obstacle_displacements(1)
                self._obstacle_displacements = tf.concat([self._obstacle_displacements, obstacle_displacement], axis=0)
        ...
        self._targets_intercepted = []
        self._target_chosen = [[] for i in range(0,num_chasers)]
        self._target_chosen = self._assign_target()
        ...
        print("Collisions: ",self._collided/num_chasers)
        print("Crashes: ",self._crashed/num_chasers)
        print("All intercepted: ",self._all_caught/num_chasers)
        print("Total no. caught: ",self._num_caught)
        print('Initial targets chosen:',self._target_chosen)
        print()
    ...
    def update(self): #Update environment after each step
        ...
        self._chaser_collided = self._has_any_within_threshold_distance(_CHASER_SELF_COLLISION_DISTANCE, self._chaser_displacements) 
        self._check_targets = self._has_any_target(_CATCHING_DISTANCE, self._chaser_displacements, self._target_displacements)
        if _HAS_OBSTACLES:
            self._chaser_hit_obstacle = self._has_any_within_threshold_distance(_CHASER_SELF_COLLISION_DISTANCE, self._chaser_displacements, self._obstacle_displacements)  #Check if any target hits obstacle
            #self._target_hit_obstacle = self._has_any_target(_CATCHING_DISTANCE, self._obstacle_displacements, self._target_displacements)  #Check if chaser hits obstacle
            self._has_collided = self._chaser_collided
            self._has_crashed = self._chaser_hit_obstacle
        else:
            self._has_collided = self._chaser_collided
        ...
        # if _HAS_OBSTACLES:
            # for i in self._target_hit_obstacle[1]:
                # if self._target_hit_obstacle[0] and (i not in self._targets_intercepted):
                    # self._targets_intercepted.append(i)
    ...
    ################ Obstacle position##########################
    def _create_initial_obstacle_displacements(self, num_obstacles: int) -> tf.Tensor:
        return self._create_initial_displacements(num_obstacles)
    ...
    def _has_any_target(self, threshold_distance: float, p_displacements: tf.Tensor, q_displacements: Optional[tf.Tensor] = None) -> tf.Tensor:
        ...
        for i in rel_dist_list:
            if i <= threshold_distance and rel_dist_list.index(i) not in target_index:
                target_index.append(rel_dist_list.index(i)%self._num_targets)   
    ...
    def _get_observation(self, agent_index: int) -> Dict[str, tf.Tensor]:
        ...
        # print()
        # print(self._target_chosen)
        # tf.print(self._target_displacements)
        # print()
        
        target_displacements = self._target_displacements[self._target_chosen[agent_index][0]]
        target_velocities = self._target_velocities[self._target_chosen[agent_index][0]]
        target_angular_velocities = self._target_angular_velocities[self._target_chosen[agent_index][0]:self._target_chosen[agent_index][0]+1]
        ...
        # [self._target_chosen[agent_index][0]]
        ...
        #Obstacle
        if _HAS_OBSTACLES:
            obstacle_displacements = self._obstacle_displacements
            obstacle_relative_displacements = obstacle_displacements - agent_displacements
            obstacle_relative_velocities = -agent_velocities
    ...
    def _calculate_distance_reward(self) -> tf.Tensor:
        ...
        # print()
        # tf.print('Targets chosen', self._target_chosen)
        # tf.print('Dist Reward', reward)
        # print()
    ...
    def _assign_target(self):
        ...
        num_chasers = len(self._chaser_ids)
        target_chosen = self._target_chosen
        
        curr_relative_displacements = find_relative_quantities(self._chaser_displacements, self._target_displacements)
        curr_relative_distances = tf.norm(curr_relative_displacements, axis=-1)
        curr_relative_distances = [i for i in curr_relative_distances]
                    
        if _ASSIGNMENT_METHOD == 1:
            for i in range(0, num_chasers):      
                target_id = i * self._num_targets
                decide_dist = curr_relative_distances[target_id:target_id+self._num_targets]
                for j in self._targets_intercepted:
                    decide_dist[j] = 10e5
                if len(target_chosen[i]) == 0:
                    target_chosen[i].append(decide_dist.index(min(decide_dist)))
        
        elif _ASSIGNMENT_METHOD == 2:
            
            theta_is = []
            theta_ts = []
            index_chaser = 0
            index_target = 0
            for i in curr_relative_displacements:   
                theta_i = int(math.atan(i[1]/i[0])) - int(self._chaser_angular_displacements[index_chaser][0])
                theta_is.append(theta_i)
                theta_t = -(int(math.atan(i[1]/i[0])) - int(self._target_angular_displacements[index_target][0]))
                theta_ts.append(theta_t)
                
                if index_target == self._num_targets-1:
                    index_target = 0
                    index_chaser += 1
                else:
                    index_target += 1
            
            vis = _CHASER_TERMINAL_SPEED
            vts = _TARGET_TERMINAL_SPEED
            ds = np.array(curr_relative_distances).reshape(num_chasers,self._num_targets)
            theta_is = np.array(theta_is).reshape(num_chasers,self._num_targets)
            theta_ts = np.array(theta_ts).reshape(num_chasers,self._num_targets)
            
            targets = [1 for i in range(0, self._num_targets)]
            
            for i in self._targets_intercepted:
                targets[i] = 0
                
            theta_is = np.delete(theta_is,self._targets_intercepted,1)
            theta_ts = np.delete(theta_ts,self._targets_intercepted,1)
            vts = np.delete(vts,self._targets_intercepted)
            ds = np.delete(ds,self._targets_intercepted,1)
            
            V = [1] * (self._num_targets - len(self._targets_intercepted))
            W = [1] * num_chasers
            ...
            result = assign.task_assignment(V,W,theta_is,theta_ts,vis,vts,ds)
            ...
            for i in range(0, num_chasers):
                index = np.nonzero(result[0][i])[0][0]
                target_index = np.nonzero(targets)[0]
                if len(target_chosen[i]) == 0:
                    target_chosen[i].append(target_index[index])
